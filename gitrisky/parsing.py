...
def get_labels():
    ...
    labels : pd.DataFrame of shape [n_commits, 1]
        The labels to use for modeling. The indexing of the dataframe matches
        the dataframe generated by `get_features()`.
    """
    ...
    labels = feats.hash.isin(bug_commits).astype(int)

    # convert to DataFrame so everything is the same type
    return labels.to_frame('label')
